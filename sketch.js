...
startImage=loadImage("Images/startbutton.jpg");
   ...
   endbgImg=loadImage("Images/bg7.jpg");
 ...
 if(gameState=="play2"){
      ...
      if(playbg.y>1000){
        playbg.y=playbg.height/2;
          ...
          if ((touches.length > 0 ||keyDown(RIGHT_ARROW)) && player.y>= height-200 ){
            player.addAnimation("moving",playerImg);
            player.x+=3;
            player.changeAnimation("moving",playerImg);
            touches=[]
          }
          if ((touches.length > 0 ||keyDown(LEFT_ARROW)) && player.y>= height-200 ){
            player.addAnimation("moving",playerImg);
            player.x-=2;
            player.changeAnimation("moving",playerImg);
          }
